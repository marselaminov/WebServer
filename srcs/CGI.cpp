...
CGI::CGI(Server *server, HttpRequest *request, std::string cgiPath) {
	...
	tmpEnv["SERVER_PORT"] = std::to_string(
			server->getPort()); // значение переменной содержит номер порта, на который был послан запрос
	tmpEnv["AUTH_TYPE"] = "basic"; // значение переменной специфицирует метод аутентификации
	tmpEnv["CONTENT_LENGTH"] = std::to_string(
			_request->getBody().size()); // значение переменной соответствует длине стандартного входного потока в символах
	...
	tmpEnv["QUERY_STRING"] = _request->get_path() + "?" +
							 _request->getParameters(); // значение переменной - строка символов после "?" в URL соответствующему данному запросу, эта инфа не декодируется сервером
